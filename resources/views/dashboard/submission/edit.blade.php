...
@section('content')
    <div class="card">
        ...
        <div class="card-content">
            <div class="card-body">
                <form action="{{ route('submission.update', $submission->id) }}" method="POST" class="form form-horizontal" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <div class="form-body">
                        <div class="row">
                            ...
                            <fieldset class=" col-md-8 form-group">
                                <select class="form-select @error('kelas_id') {{ 'is-invalid' }} @enderror" id="basicSelect" name="kelas_id">
                                    ...
                                    @foreach ($kelas as $kls)
                                        <option value="{{ $kls->id }}" @if($submission->kelas_id == $kls->id) selected @endif>{{ $kls->kelas }}</option>    
                                    @endforeach
                                </select>
                                @error('kelas_id')
                            ...
                            <div class="col-md-8 form-group">
                                <input type="number" class="form-control @error('babsubmission') {{ 'is-invalid' }} @enderror" name="babsubmission" value="{{ old('babsubmission') ?? $submission->babsubmission ?? '' }}">
                                @error('babsubmission')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-8 form-group">
                                <textarea class="form-control @error('soal') {{ 'is-invalid' }} @enderror" id="teks" rows="3" name="soal">{{ old('soal') ?? $submission->soal ?? '' }}</textarea>
                                @error('soal')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
